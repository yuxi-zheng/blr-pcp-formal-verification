/-
Copyright (c) 2025 Thomas Vidick. All rights reserved.
...
/--
The squared `σ`-seminorm.  When `σ` is positive semidefinite we use mathlib's
seminormed group instance attached to `σ`; otherwise we fall back to the real
part of `sigmaInner σ A A`.  All analytic estimates below assume `σ ≥ 0`.
-/
