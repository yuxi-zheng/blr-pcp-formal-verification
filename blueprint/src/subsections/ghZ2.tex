...
\begin{proposition}[Equivalence for BLR test]\label{prop:equiv_blr_z2}
    Let $ \Pr[\BLRVerifier=0]$ denote the probability that the BLR verifier rejects a function $f: \mathbb{F}_2^n \to \mathbb{F}_2$.
    Let $\delta(f, \lin)$ denote the distance between $f$ and the closest linear function in $\lin$.
    Then the following statements are equivalent:
    \begin{itemize}
        \item If $\Pr[\BLRVerifier=0] \leq \epsilon$, then $\delta(f, \lin) \leq \epsilon$.
        \item $\Pr[\BLRVerifier=0] \geq \delta(f, \lin)$.
    \end{itemize}
\end{proposition}    
...
\begin{proof}
    ...
    then there exists a linear function $\ell$ such that $\delta(f, \ell) \leq \epsilon$ so that $\delta(f,\lin)\le \epsilon$.

    This proves the implication 
    $\Pr[\BLRVerifier=0]\le \epsilon \Rightarrow \delta(f,\lin)\le \epsilon$,
