...
\begin{proposition}[Group isomorphism to product group]\label{prop:group_iso}\uses{lem:group}
...
\emph{Remark (formalization).} The isomorphism $\Psi$ is used here only to motivate and compute the characters. The Lean development does \emph{not} formalize $\Psi$ as a group isomorphism: it equips $G$ with its group structure (\lemref{lem:group}) and defines the characters $\chi_{\alpha,k}$ (\propref{prop:group_characaters}) directly on the twisted group, proving the homomorphism property by a direct computation rather than by pulling back through $\Psi$. Hence \propref{prop:group_iso} carries no \texttt{\textbackslash lean} link.
...
\end{proposition}
\begin{proof}\leanok
By \propref{prop:group_iso}, $(G, \star) \cong (X, +) \times (A, \cdot)$ via $\Psi(x, a) = (ax, a)$. First we consider the characters of the product group. Using Fact.~\ref{fact:pontryagin_dual}, we have 
\[
...
\begin{lemma}[Lift operation from BLR condition]\label{lem:rep-iff-blr}\uses{def:Flift}\lean{BlrPcp.liftFin_isApproxRepresentation}\leanok
...
% Then by \lemref{lem:gh_linearity_formalized}, there exists a linear function $\ell: \FF_q^n \to \FF_q$ such that the relative Hamming distance $\delta(f, \ell)$ is bounded by:
% \be\label{eq:GH_linear_bound}
