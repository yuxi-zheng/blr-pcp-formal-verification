...
\textcolor{blue}{TODO: add Trace for prime powers}

Assume $q = p$ is a prime, $n \ge 1$, and we set
\[
X := \F_q^n, \qquad A := \F_q^\times, \qquad \omega := e^{2\pi i/p}.
\]

\begin{definition}[BLR verifier and distance]\label{def:blr}
...
\begin{theorem}[Soundness of the BLR test]\label{thm:main}
For every $f : \F_q^n \to \F_q$, $\delta(f, \lin) \le \Pr_{a,b,x,y}\!\bigl[af(x) + bf(y) \ne f(ax + by)\bigr]$.
\end{theorem}
...
\begin{lemma}[$G$ is an abelian group]\label{lem:group}
...
\begin{proposition}[Group isomorphism to product group]\label{prop:group_iso}
    Let $(G, \star)$ be an abelian group defined by Equation~\ref{eq:star}. The map
\be
\Psi : G \to (X, +) \times (A, \cdot), \qquad
\Psi(x, a) := (ax, a),
\ee
is a group isomorphism, where the codomain is the direct product of $(X, +)$ and
$(A, \cdot)$ with operation $\star_\Psi:(x,\; a) \star_\Psi (y,\; b) = (x+y,\; ab)$. 
% In particular $|G| = q^n(q-1)$.
\end{proposition}
\begin{proof}
    Compute
\begin{align*}
\Psi\bigl((x, a) \star (y, b)\bigr) = \Psi(b^{-1}x + a^{-1}y,\; ab)= \bigl(ab\cdot(b^{-1}x + a^{-1}y),\; ab\bigr) = (ax + by,\; ab),
\end{align*}
which equals $\Psi(x, a) \star_\Psi \Psi(y, b) = (ax + by,\; ab)$. The map $\Psi$ is bijective with
inverse $\Psi^{-1}(y, b) = (b^{-1}y, b)$.
\end{proof}
...
\begin{fact}[Pontryagin dual over direct product]\label{fact:pontryagin_dual}
    Let $G_1, G_2$ be finite abelian groups. Let $H = G_1 \times G_2$ be their direct product. Then the Pontryagin dual formed by all the irreps. is $\hat{H} = \hat{G_1} \times \hat{G_2}$.
\end{fact}
\begin{proposition}[Group characters]\label{prop:group_characaters}
The group $(G, \star)$ has the Pontryagin dual character group 
\be
\hat{G} = \{\chi_{\alpha, k}\}_{\alpha \in X, k \in [0, \cdots, q-2]}, \quad \chi_{\alpha, k}(x,a) = \omega^{\inner{\alpha}{a x}} \psi_k(a),
\ee
where $\psi_k(a) = e^{\frac{i 2 \pi m k}{q-1}}$ and $a=g_0^m$ for a fixed multiplicative generator $g_0\in A$.
\end{proposition}
\begin{proof}
By Proposition.~\ref{prop:group_iso}, $(G, \star) \cong (X, +) \times (A, \cdot)$ via $\Psi(x, a) = (ax, a)$. First we consider the characters of the product group. Using Fact.~\ref{fact:pontryagin_dual}, we have 
\[
\widehat{(X, +) \times (A, \cdot)} = \hat{X} \times \hat{A}.
\]
The character groups of $X$ and $A$ are
\begin{align}
    \hat{X} &= \{\phi_\alpha(y) = \omega^{\inner{\alpha}{y}}\}_{\alpha\in X}, \\
     \hat{A} &= \{\psi_k(b) =e^{\frac{i 2 \pi m k}{q-1}}\}_{k \in [0, \cdots, q-2]},
\end{align}
where $b=g_0^m$ for a fixed multiplicative generator $g_0\in A$. Therefore, $\widehat{(X, +) \times (A, \cdot)} = \{\Theta_{\alpha, k}(y, b) = \phi_\alpha(y) \psi_k(b)\}_{\alpha, k}$.
...
\ee
Therefore, we have $\chi_{\alpha, k}(x, a) = \phi_\alpha(ax) \psi_k(a) =\omega^{\inner{\alpha}{a x}} \psi_k(a)$.
\end{proof}
...
F_f(x, a) = D(a f(x)) := \sum_{c \in A} \omega^{c a f(x)}\ket{c}\bra{c},
\end{equation}
...
\begin{lemma}[Lift operation from BLR condition]\label{lem:rep-iff-blr}
...
\begin{align*}
F_f\bigl((x,a)\star(y,b)\bigr) &= \sum_{c\in A}\omega^{cab\,f(b^{-1}x + a^{-1}y)} \ket{c}\bra{c}, \\
F_f(x, a)\,F_f(y, b) &= \sum_{c\in A} \omega^{c(af(x) + bf(y))} \ket{c}\bra{c}.
\end{align*}
These coincide iff their exponents agree modulo $p$ \textcolor{red}{Check this is true if we add Trace}: 
\[
...
\begin{align}
    ...
    & = \mathbb{E}_{(x,a), (y,b) \in G} \frac{1}{q-1}\Re \Tr\left[ \sum_{c\in A} \omega^{-c(af(x) + bf(y))} \omega^{cab\,f(b^{-1}x + a^{-1}y)} \ket{c}\bra{c}\right], \nonumber \\
    & = \mathbb{E}_{(x,a), (y,b) \in G} \frac{1}{q-1}\Re \sum_{c\in A} \omega^{c z},
\end{align}
where we define $z=c (ab\,f(b^{-1}x + a^{-1}y) - (af(x) + bf(y)))$. To compute the expectation, we can split to two cases
\begin{align}
\sum_{c\in A} \omega^{c z} = 
    \begin{cases}
        q-1, \quad z=0 \quad \text{when $f$ passes BLR}, \\
        -1, \quad z \neq 0 \quad \text{when $f$ fails BLR}.
    \end{cases}
\end{align}
The second case is because $\sum_{c\in A} \omega^{c z} + 1= \sum_{c\in \FF_q} \omega^{c z} =0$. Therefore, we have the approximate representation
\be
...
\end{equation}
    ...
    Because $F_f(g)$ is diagonal in $\{\ket{c}\}$ with entries $\omega^{caf(x)}$, we can rewrite the inner product trace:
    \[
        \langle F_f(g), V^* R(g) V \rangle_\sigma = \frac{1}{q-1} \sum_{c \in \FF_q^\times} \langle c | F_f^*(g) V^* R(g) V | c \rangle = \frac{1}{q-1} \sum_{c \in \FF_q^\times} \omega^{-caf(x)} \langle v_c, R(g) v_c \rangle_{L(G, \mathcal{H})},
    \]
    ...
    \begin{equation}
        \frac{1}{q-1} \sum_{c \in \FF_q^\times} \EE_{x, a} \Re \left[ \omega^{-caf(x)} \langle v_c, R(x, a) v_c \rangle \right] \ge 1 - \epsilon. \label{eq:expanded}
    \end{equation}

    A basis for $L(G, \mathcal{H})$ is given by $\{\chi \ket{c}\}_{\chi\in\hat{G}, c\in \FF_q^\times}$. By Proposition.~\ref{prop:group_characaters}, the characters are given by
    \[
    \hat{G} = \{\chi_{\alpha, k}\}_{\alpha \in X, k \in [0, \cdots, q-2]}, \chi_{\alpha, k}(x,a) = \omega^{\inner{\alpha}{a x}} \psi_k(a).
    \]
    ...
    \[
        \sum_{c \in \FF_q^\times} \left( \frac{1}{q-1} \right) \sum_{\alpha, k} p_{c, \alpha, k} \Re \left( \EE_{x, a} \left[ \omega^{-caf(x)} \chi_{\alpha, k}(x, a) \right] \right) \ge 1 - \epsilon.
    \]
    ...
    \begin{equation}
        \Re \left( \EE_{x, a} \left[ \omega^{-c^*af(x)} \omega^{\langle \alpha^*, ax \rangle} \psi_{k^*}(a) \right] \right) \ge 1 - \epsilon. \label{eq:dominant}
    \end{equation}
    ...
    \[
        E^* := \EE_{x} \left[ \EE_{a} \left[ \omega^{c^* a (\ell(x) - f(x))} \psi_{k^*}(a) \right] \right].
    \]
    ...
    \be
    \EE_{a \in \in \FF_q^\times}\omega^{c^* a (\ell(x) - f(x))} = 
    \begin{cases}
        1, \quad z(x) = 0, \\
        - \frac{1}{q-1}, \quad z(x) \neq 0,
    \end{cases}
    \ee
    where in the second case we have again used average of roots of unity is $\sum_{b \in \in \FF_q^\times} \omega^{b} +1 = 0$.
    ...
    \item Consider $k^* \neq 0$, then by Proposition.~\ref{prop:group_characaters} we have $\psi_k(a) = e^{\frac{i 2 \pi m k}{q-1}}$ and $a=g_0^m$ for a fixed multiplicative generator $g_0\in \FF_q^\times$. Note that averaging over $\psi_k(a)$ we have
    ...
    \be
    \EE_{a \in \in \FF_q^\times}\omega^{c^* a (\ell(x) - f(x))}  \psi_{k^*}(a) = 
    \begin{cases}
        0, \quad z(x) = 0, \\
        \frac{1}{q-1} \sum_{a \in \in \FF_q^\times} \omega^{a z(x)}\psi_{k^*}(a), \quad z(x) \neq 0.
    \end{cases}
    \ee
    In the second case, let $b = a z(x)$ so that
    \begin{align}
        \frac{1}{q-1} \sum_{a \in \in \FF_q^\times} \omega^{a z(x)}\psi_{k^*}(a) &= \frac{1}{q-1} \sum_{b \in \in \FF_q^\times} \omega^{b}\psi_{k^*}(z^{-1}b), \nonumber \\
        & =  \frac{\psi_{k^*}(z^{-1})}{q-1} \sum_{b \in \in \FF_q^\times} \omega^{b}\psi_{k^*}(b).
    \end{align}
    It turns out the summation is a known quantitiy called \textit{the Gauss sum}
    \be
    \mathcal{G}(\psi_{k^*}) = \sum_{b \in \in \FF_q^\times} \omega^{b}\psi_{k^*}(b).
    \ee
...
\begin{proposition}[Fourier coefficient of $F_f$]\label{prop:Ff_fourier}
...
\begin{proof}
    By Proposition.~\ref{prop:group_characaters}, the characters are given by
    \[
    \hat{G} = \{\chi_{\alpha, k}\}_{\alpha \in X, k \in [0, \cdots, q-2]}, \chi_{\alpha, k}(x,a) = \omega^{\inner{\alpha}{a x}} \psi_k(a).
    \]
    ...
    \begin{align}
        ...
        & = \sum_{c \in \FF_q^\times } \EE_{x, a} \omega^{caf(x)} \bar{\chi}_{\alpha, k}(x, a) , \nonumber \\
        & = \sum_{c \in \FF_q^\times }\EE_{x,a} \left[ \omega^{a(cf(x) - \langle \alpha, x \rangle)} \overline{\psi_k(a)} \right] \ket{c}\bra{c}.
    \end{align}
...
\begin{align} 
    [\hat{F}_f(\chi_{c\beta, k})]_{cc} &= \EE_{x,b} \left[ \omega^{b(f(x) - \langle \beta, x \rangle)} \overline{\psi_k(b)}\psi_k(c) \right] \nonumber \\
    &= \psi_k(c) \nu_k(\beta), 
\end{align}
where $\nu_k(\beta) := \EE_{x,b} \left[ \omega^{b(f(x) - \langle \beta, x \rangle)} \overline{\psi_k(b)} \right]$ is a $c$-independent coefficient.    
\end{proof}


\begin{lemma}[Gowers-Hatami to linearity (Convex character Fourier hybrid)] \label{lem:gh_linearity_3}
Suppose $F_f: G \to U(\cc^{q-1})$ defined in Equation~\ref{eq:Flift-def} via $f : \FF_q^n \to \FF_q$ is an $(\epsilon, \sigma)$ approximate unitary representation of $(G, \star)$ defined in Equation~\ref{eq:star}. 
Then, there exists a linear function $\ell: \FF_q^n \to \FF_q$ such that the relative Hamming distance $\delta(f, \ell)$ is bounded by:
\be\label{eq:GH_linear_bound}
    \delta(f, \ell) \le \frac{q-1}{q} \epsilon.
...
\end{equation}
    ...
    \ee
    By Proposition.~\ref{prop:Ff_fourier}, let $\alpha = c \beta$, we can show
    \be
    ...
    \begin{align*}
            E &:= \frac{1}{q-1} \sum_{c \in \FF_q^\times} \EE_{x, a} \Re \left[ \omega^{-caf(x)} \langle v_c, R(x, a) v_c \rangle \right], \\
            % & = \sum_{c \in \FF_q^\times} \left( \frac{1}{q-1} \right) \sum_{\alpha, k} p_{c, \alpha, k} \Re \left( \EE_{x, a} \left[ \omega^{-caf(x)} \chi_{\alpha, k}(x, a) \right] \right), \\
            & = \frac{1}{q-1} \sum_{c \in \FF_q^\times}\sum_{\alpha, k} p_{c, \alpha, k} \Re \left( \EE_{x, a} \left[ \omega^{-caf(x)} \omega^{\inner{\alpha}{ax}} \psi_k(a) \right] \right).
    \end{align*}

    Relabel the sum by $\alpha = c \beta$ and $b=ca$, we have
    \[
    E =  \frac{1}{q-1} \sum_{c \in \FF_q^\times}\sum_{\beta, k} \abs{\nu_k(\beta)}^2 \Re \left( \EE_{x, b} \left[ \omega^{b(\inner{\beta}{x} - f(x))} \psi_k(c^{-1})\psi_k(b) \right] \right).
    \]
    ...
    we have     
    \[
    E = \sum_{\beta} \abs{\nu_0(\beta)}^2 \Re \left( \EE_{x, b} \left[ \omega^{b(\inner{\beta}{x} - f(x))} \right] \right).
    \]
    ...
    \begin{equation}
        E^* := \Re \left( \EE_{x, b} \left[ \omega^{b(\inner{\beta^*}{x} - f(x))} \right] \right) \ge 1 - \epsilon. \label{eq:dominant}
    \end{equation}
    
    Let $\ell_\beta(x):=\inner{\beta^*}{x}$, the inner expectation over $b \in \FF_q^\times$ for a fixed $x$:
        \be
        \EE_{b \in \in \FF_q^\times}\omega^{b z(x)} = 
        \begin{cases}
            1, \quad z(x) = 0, \\
            - \frac{1}{q-1}, \quad z(x) \neq 0,
        \end{cases}
