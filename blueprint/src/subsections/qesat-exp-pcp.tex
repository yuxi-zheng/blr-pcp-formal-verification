...
\begin{enumerate}
  ...
  \item Check that \(V_{\LINEQ(\F_2,m,n)}^{\langle \pi, \cdot \rangle } (M_{x},c_{x}) = 1\).
  \item Query \(\pi\) at
  ...
  [Vincent: TODO: the last check actually uses QESAT.tensorSelfVerifier, it would be nice to
  make TENSORQ.verifier work directly for our use case, to improve modularity and avoid duplicate code.]
\end{enumerate}
\end{definition}
